...
class ChromaRetriever:
    ...
    def __init__(self, collection_name: str = "memories", 
        ...
        if embedding_function:
             ...
             chroma_ef = None 
             self.external_embedding_function = embedding_function # Store for potential use? Unlikely needed if passing embeddings.
             ...
             logger.warning("ChromaDB using default embedding function. Custom LLM embeddings will not be used directly by Chroma.")

        logger.info(f"Getting or creating Chroma collection: {collection_name}")
        self.collection = self.client.get_or_create_collection(
            name=collection_name, 
            embedding_function=chroma_ef # Pass EF if using Chroma's embedding
        )
        ...
        for i, retriever in enumerate(self.retrievers):
            try:
                ...
                if 'task_type' in retriever.search.__code__.co_varnames:
                    results = retriever.search(query, top_k * 2, task_type=task_type)  # Get more results for better reranking
                else:
